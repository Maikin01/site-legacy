...
function Home() {
  ...
  useEffect(() => {
    const exitStateKey = 'legacyExitIntentArmed';
    const currentState = window.history.state ?? {};

    if (!currentState[exitStateKey]) {
      window.history.pushState({ ...currentState, [exitStateKey]: true }, '', window.location.href);
    }
    ...
    return () => window.removeEventListener('popstate', handleFirstBack);
  }, []);
